...
工作流 `.github/workflows/desktop-portable.yml` 继续在 `windows-2025` 上构建 Windows 便携版。PR 和手动运行工作流时，还会在 Ubuntu 上并行启动独立的 Android ARM64 job：它从 `ANDROID_KEYSTORE_BASE64`、`ANDROID_KEYSTORE_PASSWORD`、`ANDROID_KEY_ALIAS` 与 `ANDROID_KEY_PASSWORD` Actions Secrets 恢复 release keystore，验证 APK 签名，并上传已签名 APK 和适用于 Play 的 AAB。工作流进入默认分支后，可以用 `gh workflow run desktop-portable.yml --ref <branch>` 或 GitHub Actions 页面同时触发两种打包。
