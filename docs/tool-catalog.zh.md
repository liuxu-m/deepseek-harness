...
| `@deepseek-ai/dsh-tool-subagent-control` | `close_agent`、`followup_task`、`get_agent_status`、`interrupt_agent`、`list_agents`、`send_message`、`steer_agent`、`wait_agent` | `ctx.tools`、`ctx.subagents`、`ctx.agents and ctx.sessionProjections (list_agents only)` | `tool/call`、`tool/result`、`child session events through ctx.subagents` | - | 这些是控制可继续后台 subagent 的全局命名工具：绑定提供方的 `tool-subagent` 实例注册不同的委派工具；本包注册 queue、follow-up、steering、中断、关闭、状态和基于 inbox 的 wait 控制，另由 `list_agents` 通过单独加载的 `/list-agents` 插件提供，其目录行使用 sessionProjections 和实时 Agent 注册表。 |
...
```json
{
  ...
  "properties": {
    "agent_id": {
      "type": "string",
      "description": "The target subagent id."
    },
    "cascade": {
      "type": "boolean",
      "description": "Close descendants as well; defaults to true.",
      "default": true
    }
  },
  "required": [
...
在运行中 subagent 的下一安全 step 投递纠偏消息。
...
这些是控制可继续后台 subagent 的全局命名工具：绑定提供方的 `tool-subagent` 实例注册不同的委派工具；本包注册 queue、follow-up、steering、中断、关闭、状态和基于 inbox 的 wait 控制，另由 `list_agents` 通过单独加载的 `/list-agents` 插件提供，其目录行使用 sessionProjections 和实时 Agent 注册表。
