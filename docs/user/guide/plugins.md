...
Install the historical token usage page and the workspace Explorer and Preview panels:
...
The installed versions in the current profile are `dsh-token-usage` `0.2.2` and `@linxin666/dsh-client-ui-aionui-panel` `0.1.20`. Package resolution may select newer versions when the profile lockfile is not copied.
