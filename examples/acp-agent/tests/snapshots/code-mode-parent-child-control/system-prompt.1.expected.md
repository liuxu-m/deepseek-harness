...
interface ToolArgsMap {
  ...
  /** Close a subagent and, by default, its owned descendants. */
  close_agent: {
    /** The target subagent id. */
    agent_id: string;
    /** Close descendants as well; defaults to true. */
    cascade?: boolean;
  } & Record<string, JsonValue>;
  ...
  /** Queue a message for a background subagent and wake it to process the next turn. */
  followup_task: {
    /** The target subagent id. */
    subagent_id: string;
  ...
  /** Read the current status snapshot for an authorized subagent. */
  get_agent_status: {
    /** The target subagent id. */
    agent_id: string;
  } & Record<string, JsonValue>;
  ...
  /** Queue a message as the subagent next turn without waking it. Use followup_task to queue and wake it. */
  send_message: {
  ...
  /** Deliver a message to a running subagent at its next safe step. */
  steer_agent: {
    /** The target subagent id. */
    agent_id: string;
  ...
  /** Wait until a new message is pending for your next step, or until timeout_ms expires. This does not read, return, or remove the message. When it returns Wait completed., end this step immediately: the message becomes visible in the next step. */
  wait_agent: {
    /** Maximum milliseconds to wait. Defaults to 120000. */
    timeout_ms?: number;
  } & Record<string, JsonValue>;
...
interface ToolOutputMap {
  ...
  followup_task: {
    ...
    accepted: boolean;
    messageId?: string;
    effectiveStep?: "next-step";
    previousState?: string;
  };
  get_agent_status: {
    agentId: string;
    parentSessionId: string;
    state: "running" | "idle" | "ready" | "completed" | "failed" | "interrupted" | "closed";
