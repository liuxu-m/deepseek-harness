...
`GET /api/runtime.identity` 返回供 Desktop shell 判断能否附着到运行中 Web Host 的公开记录 `{product, desktopProtocol, version, instanceId, homeKind}`。该路由只接受 GET，使用 `cache-control: no-store`，且绝不返回 Harness home 的绝对路径。`globalImage` 允许 Web 和 Desktop 部署在所选模型未声明图像输入时接收图像；请求路径会把这些图像块替换为文本占位符，因此必须由外部视觉工具解读图像。内置 Web patch 将 `globalImage` 设为 `true`；用户 patch 可以设为 `false`，恢复基于模型声明的图像限制。
