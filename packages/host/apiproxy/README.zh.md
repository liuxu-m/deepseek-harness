...
`session.prompt` 的图片准入以及含图片会话内的模型切换由 `globalImage` 服务门控（[Web profile 标志](../../../.agents/notes/implemented/feature/2026-08-14-global-image-web-profile.zh.md)）：开启时，带图片的提示词仅凭持久化附件服务即可准入（逐模型的 `inputModalities` 限制与待处理图片的切换限制对整个部署解除），关闭时保留原有的模型声明检查不变。
