...
A provider route may also set `reasoningSummary` (`'auto' | 'detailed' | 'concise' | null`) to override the `reasoning.summary` it sends on OpenAI Responses routes. It is opt-in: only a route that sets it installs the override hook, so unset routes keep pi-ai's default `"auto"`. Some Codex-compatible gateways' upstreams only accept `"detailed"` (returning `400 upstream_error` for `"auto"`), so setting `reasoningSummary: detailed` aligns that route with stock Codex.

A provider route may additionally set `omitMaxTokens: true` to drop the `max_output_tokens` parameter from its OpenAI Responses requests. Verified 2026-08-21: the icode `gpt-5.6-terra` upstream returns `400 upstream_error` for *any* request carrying `max_output_tokens` — regardless of value (64 / 2048 / 32768 all rejected) — while the exact same request with that one field omitted succeeds. Stock Codex never sends it. Opt-in only; unset routes keep pi-ai's normal output cap.
