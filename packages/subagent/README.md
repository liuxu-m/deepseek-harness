...
| [`tool-subagent-control/`](tool-subagent-control/README.md) | Exposes queue, follow-up, steering, interruption, closure, status, and listing to the model | registers on `ctx.tools` |
...
Continuable controls use durable ids and live-parent authority. Queue, follow-up, and steering deliver messages rather than results; `report` is optional and child-scoped. Hosts may project bounded reports and lifecycle receipts without exposing private prompts or complete transcripts.
