...
describe('Desktop portable workflow', () => {
  ...
  it('builds ARM64 Android APK and AAB for pull requests and manual package runs', () => {
    ...
    expect(steps.some(step => hasRun(step, 'apps/desktop/node_modules/.bin'))).toBe(true)
    ...
    const signing = steps.find(step => hasRun(step, 'ANDROID_KEYSTORE_BASE64'))
    expect(signing).toBeDefined()
    expect(hasRun(signing!, 'base64 --decode')).toBe(true)
    expect(hasRun(signing!, 'signingConfigs')).toBe(true)
    const verifyIndex = steps.findIndex(step => hasRun(step, 'apksigner'))
    const buildIndex = steps.findIndex(step => hasRun(step, 'tauri android build --apk --aab --target aarch64'))
    expect(verifyIndex).toBeGreaterThan(buildIndex)
  })

  it('uploads only explicitly named signed Android release artifacts', () => {
    const workflow = loadWorkflow(WORKFLOW_PATH)
    const job = androidJob(workflow)
    const steps = stepsOf(job)
    const upload = steps.find(step => typeof step.uses === 'string' && step.uses.startsWith('actions/upload-artifact@'))
    const verify = steps.find(step => hasRun(step, 'verify --verbose --print-certs'))
    expect(upload).toBeDefined()
    expect(verify).toBeDefined()
    expect(hasRun(verify!, '*-unsigned.apk')).toBe(true)
    const path = String(stepWith(upload!)['path'])
    expect(path).toContain('app-universal-release.apk')
    expect(path).toContain('app-universal-release.aab')
    expect(path).not.toContain('*')
    expect(path).not.toContain('unsigned')
