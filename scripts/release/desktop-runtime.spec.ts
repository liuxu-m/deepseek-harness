...
describe('desktop runtime deploy plan', () => {
  it('orders build, closure verification, then a production desktop deploy', () => {
    ...
    expect(planDesktopRuntime({ stage })).toEqual([
      ...
      { command: 'pnpm', args: ['run', 'verify-runtime-closure', '--manifest', 'apps/desktop-tauri/package.json'] },
      {
        command: 'pnpm',
        args: [
          '--filter', '@deepseek-ai/dsh-desktop-tauri',
          'deploy', '--legacy', '--prod',
  ...
  it('resolves the repository root two levels above scripts/release', async () => {
    ...
    await expect(readFile(join(REPO_ROOT, 'apps/desktop-tauri/package.json'), 'utf8')).resolves.toContain('@deepseek-ai/dsh-desktop-tauri')
  })
})
...
describe('legacy deploy hoist restore', () => {
  it('copies a missing direct dependency from the deploy root node_modules', async () => {
    ...
    await writeFile(
      ...
      JSON.stringify({ name: '@deepseek-ai/dsh-desktop-tauri', version: VERSION, dependencies: { '@deepseek-ai/cosmokit': 'workspace:^' } }, null, 2) + '\n',
    )
    const source = join(fixture.root, 'apps/desktop-tauri/node_modules/@deepseek-ai/cosmokit')
  ...
  it('skips the source package nested node_modules so cyclic self-links never recurse', async () => {
    ...
    await writeFile(
      join(fixture.stage, 'package.json'),
      JSON.stringify({ name: '@deepseek-ai/dsh-desktop-tauri', version: VERSION, dependencies: { '@deepseek-ai/dsh': 'workspace:^' } }, null, 2) + '\n',
    )
    ...
    const source = join(fixture.root, 'apps/desktop-tauri/node_modules/@deepseek-ai/dsh')
    await mkdir(join(source, 'lib'), { recursive: true })
  ...
  it('reports a dependency absent from both the stage and the deploy root', async () => {
    ...
    await writeFile(
      join(fixture.stage, 'package.json'),
      JSON.stringify({ name: '@deepseek-ai/dsh-desktop-tauri', version: VERSION, dependencies: { '@deepseek-ai/absent': 'workspace:^' } }, null, 2) + '\n',
    )
    try {
